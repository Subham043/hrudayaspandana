...
class Services extends CI_Controller {
    ...
    public function audio($id='', $type='')
	{
		...
		if(!empty($id) && !empty($data['service'])){
            ...
            $this->load->view('pages/services/view-services-audio', $data);  
		}else{
            $this->session->set_flashdata('error','Invalid Page');
            return redirect('services/'.$type);
		}
    ...
    public function galleryAudioUpload($id='')
	{
        $config['upload_path']          = '../assets/images/home/audio';
        $config['allowed_types']        = 'mp3|aac|wav';                
        $config['max_width']            = 0;
        $config['encrypt_name']         = TRUE;
        $this->load->library('upload', $config);
        if(!is_dir($config['upload_path'])) mkdir($config['upload_path'], 0777, TRUE);
        ...
        if ( ! $this->upload->do_upload('files'))
        {
            ...
            $application_id = $this->m_services->insertGalleryAudio($newdata);
            if($application_id != false)
            {
                $data = array(
                    'success'=>true
    ...
    public function galleryAudioView($id='')
	{
        $id = $this->encryption_url->safe_b64decode($id);
        $data['result']= $this->m_services->serviceAudioGetById($id);
        echo json_encode($data);
    }
